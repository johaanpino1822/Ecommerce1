...
// Función mejorada para obtener el token de aceptación con reintentos
async function getAcceptanceToken(retries = 3) {
  for (let i = 0; i < retries; i++) {
    try {
      const response = await axios.get('https://sandbox.wompi.co/v1/merchants/pub_test_Y5wO75g5gGwby7LHpmcerjQKOn6EnJSq', {
        timeout: 5000
      });
